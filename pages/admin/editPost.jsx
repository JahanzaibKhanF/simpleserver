...
export default authHOC(function editPost () {
 
 
  
  useEffect(() => {
    const localStorageData = localStorage.getItem('editeData');
  
   setSocietiesData(JSON.parse(localStorageData))
   console.log("here",JSON.parse(localStorageData))
  }, []);
...
const postData = async () => {
  if (societiesData.title === "" || societiesData.title === null || societiesData.title.length < 10) {
    ...
    setTitleError(false);
    fetch(`/api/societies?_id=${societiesData._id}`, {
      method: 'PATCH',
      headers: {
    ...
    .then(response => {
      if(response.status===200){
        localStorage.removeItem('editeData')
        router.push('/admin')
      }
      else{
        console.log('Error Updating society ....');
        setApiError(true);
      }
      response.json()
    })
    .then(data => {
      console.log('Response from Updating society:', data);
  ...
  return (
    <div className='w-full bg-black  h-full'>
      ...
      <h1 className='text-3xl font-bold text-white text-center py-10'>Edite Society</h1>
...
<button onClick={postData} className='bg-sky-500 text-white py-2 w-[150px] hover:text-white hover:bg-sky-500 sm:w-[200px] mx-auto'>Update</button>
    
    {apiError && <p className='text-red-500'>Error while Updating : Server Error or Check If Post Already Exists</p>}
     </div>
