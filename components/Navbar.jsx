        ...
        <div onClick={handleCloseMenu} className="max-w-full    ">
            ...
            <div className=' lg:hidden  w-[200px] md:w-[300px] overflow-hidden'><Link href="/"><img src="/images/logo.jpg" className='w-full ' /></Link></div>
              <div className='hidden lg:block'>  <ul className=" text-white  flex gap-10  lg:py-[70px]">
                ...
                <div className="hidden group-hover:block absolute text-white bg-[#111211] shadow-lg rounded-lg px-5 py-5 w-[300px]">
                <ul className="w-full">
                 {
                    societiesData.map((item)=>{
                        return(
                            <>
                            <li className="my-4 hover:text-[#fce303]  pl-2 border-b-[1px] py-2 border-slate-600   "><Link href={`/societies/${item.url}`}>{item.title}</Link></li>
                            </>
                        )
                    })
                 }
            ...
            <div onClick={handelMenu} className="py-2 md:py-6 lg:py-8 lg:hidden">{isMenuOpen ? (<AiOutlineClose className={` text-white`} size={50}/>):(<FiMenu  className="text-white" size={50}/>)}</div>
             ...
             {/* small menu */}
             <div onClick={(e)=>e.stopPropagation()} className={`block lg:hidden bg-gray-900 absolute  lg:h-1  h-full w-[70%]  shadow-lg shadow-black transition-all duration-3000 
              ${isMenuOpen? "top-10  h-[452px] ":" bottom-[600px] "} right-[15%] md:right-0 pt-[70px] md:pt-[120px]  `}>  <ul className=" text-white bg-gray-900 rounded-xl px-2  ">
                <li className="py-5 border-b border-gray-50  px-2 hover:bg-sky-700"><Link href="/">HOME</Link></li>
                {/* <li className="py-5 border-b border-gray-50 px-2 hover:bg-sky-700">BLOG</li> */}
                {/* <li className="py-5 border-b border-gray-50 px-2 hover:bg-sky-700">VIDEOS</li> */}
                <li className="py-5 border-b border-gray-50 px-2 hover:bg-sky-700 group relative"><Link href="/societies">SOCIETIES</Link>
                <div className="hidden group-hover:block absolute text-white bg-[#111211] rounded-xl shadow-lg px-5 py-10 w-[300px]  sm:w-[400px]">
                <ul className="w-full">
                 {
                    societiesData.map((item)=>{
                        return(
                            <>
                            <li className="py-4 border-b border-gray-50 hover:text-[#fce303]  hover:border-[#fce303] hover:border-l-4 pl-2    "><Link href={`/societies/${item.url}`}>{item.title}</Link></li>
                            </>
                        )
                    })
                 }
